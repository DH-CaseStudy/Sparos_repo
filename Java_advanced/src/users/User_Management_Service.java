...
public class User_Management_Service {
    ...
    private void insertMember(){
        ...
        System.out.print("나이 >> ");
        int memTel = Integer.parseInt(scan.next());

        scan.nextLine();  // 입력 버퍼 비우기
        System.out.print("이메일 >> ");
        String memreamil = scan.nextLine();
    ...
    private void updateMember(){
        ...
        int newMemAge = Integer.parseInt(scan.next());
        ...
        try {
            ...
            String sql = new StringBuilder().append(" UPDATE users SET ")
                    ...
                    .append(", userage = ? ").append(", useremail = ? ")
                    .append(" WHERE userid = ? ").toString();
    ...
    private int getMemberCount(String memId){
        ...
        try {
            conn = DBUtil.getConnection();
            String sql = new StringBuilder().append("SELECT COUNT(*) FROM users WHERE userid = ? ").toString();

            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, memId);
            rs = pstmt.executeQuery();

            if (rs.next()) {  // ✅ 한 번만 실행하도록 변경
                count = rs.getInt(1); // ✅ `COUNT(*)` 값 가져오기
            }
